<template>
  <div class="position-relative">
      ...
      <div style="overflow-y:auto">
        <b-table small stacked="sm" :fields="FieldTableItems" :items="filteredItemsData" :sort-by.sync="querySortBy" :sort-desc.sync="querySortDesc" @sort-changed="sortingChanged" :current-page="currentPage" :per-page="perPage">
          ...
          <template v-slot:cell(membership)="data">
            <span v-if="data.item.membership_id == 0">
              <b-badge :variant="getBadgeMembership(data.item.membership)" class="p-1">
                REGULAR
              </b-badge>
            </span>
            <span v-if="data.item.membership_id > 0">
              <b-badge :variant="getBadgeMembership(data.item.membership.name)" class="p-1">
    ...
    <b-alert show v-if="isLoaded && itemsData.length == 0">
      ...
      <hr>
      <div>
        The company with ACTIVES status and REGULAR MEMBERSHIP will be shown here
      </div>
    </b-alert>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        axios.get(`/api/companies/regular`)
          .then((response) => {
            this.isLoaded = true
